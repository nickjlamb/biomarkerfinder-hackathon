    ...
    try {
      ...
      const biomarkerSummaries = await Promise.all(
        rows.map(async (item) => {
          ...
          const literature = item.target.evidences.rows
            .flatMap(row => row.literature || []);           
...
"Unknown: There is insufficient evidence to classify this biomarker..."`;
          ...
          const result = {
            ...
            evidence_count: item.target.evidences.count,
            datatypes: item.datatypeScores,
            datasources: item.datasourceScores,
