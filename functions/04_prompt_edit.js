    ...
    try {
      ...
      const biomarkerSummaries = await Promise.all(
        rows.map(async (item) => {
          ...
          const summaryPrompt = `Explain the role of the biomarker ${geneName} in the context of ${diseaseName} in 1-2 sentences. 
          Only use information from peer-reviewed publications, and include a summary of confidence intervals and the number of supporting studies. 
          Add a sentence on any differences in the prevalence of the biomarker between populations, if any. 
          Make it accessible to non-experts.`;

          const clinicalPrompt = `State the clinical relevance of the biomarker ${geneName} in the context of ${diseaseName} in 1 sentence. 
          Only use information from peer-reviewed publications. 
          Make it accessible to non-experts.`;

          const mechanismPrompt = `Give a single-sentence summary of the specific mechanism of action of biomarker ${geneName} in ${diseaseName}. 
          Use information from peer-reviewed publications. 
          Make it accessible to non-experts.`;
...
"Unknown: There is insufficient evidence to classify this biomarker..."`;
          ...
          try {
            ...
            clinical = clinicalRes.choices[0].message.content.trim();
            mechanism = mechanismRes.choices[0].message.content.trim();
